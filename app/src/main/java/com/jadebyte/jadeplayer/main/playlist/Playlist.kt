...
data class Playlist(val id: Long, var name: String, val modified: Long, var songsCount: Int = 0) : Parcelable {
    ...
    /**
     *  When [width] is more than [Constants.MAX_PLAYLIST_IMAGE_THUMB_WIDTH], we'll change the value of any of this
     ...
     *  than [Constants.MAX_PLAYLIST_IMAGE_THUMB_WIDTH]. See [PlaylistModelLoader] for the algorithm
     *
     *  @param width the width of the ImageViw in pixels
     *  @return this playlist
     */
    fun modForViewWidth(width: Int): Playlist {
        if (width.dp > Constants.MAX_PLAYLIST_IMAGE_THUMB_WIDTH) {
